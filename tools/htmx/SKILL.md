...
1. Download <a download href="https://cdn.jsdelivr.net/npm/htmx.org@4.0.0/dist/htmx.min.js">htmx.min.js</a>
...
- <a download href="https://cdn.jsdelivr.net/npm/htmx.org@4.0.0/dist/htmx.js">htmx.js</a> (unminified)
- <a download href="https://cdn.jsdelivr.net/npm/htmx.org@4.0.0/dist/htmx.esm.min.js">htmx.esm.min.js</a> (ES module)
- <a download href="https://cdn.jsdelivr.net/npm/htmx.org@4.0.0/dist/htmx.esm.js">htmx.esm.js</a> (ES module, unminified)
...
htmx has many useful [extensions](https://four.htmx.org/extensions) that add functionality to it.

If you want to install a distribution that ships with most of the useful extensions already installed so you
don't have to think about it, you can use the [htmax.js](https://four.htmx.org/docs/htmax) distribution.
...
- In htmx 2.0 attribute inheritance is _implicit_ by default while in 4.0 it is _explicit_ by default
  - To restore the 2.0 behavior, you can set the [`htmx.config.implicitInheritance`](https://four.htmx.org/reference/config/htmx-config-implicitInheritance) setting to `true`
- In htmx 2.0, `400` and `500` response codes are _not_ swapped by default, whereas in htmx 4.0 these requests _will_ be
  swapped
  - To restore the 2.0 behavior, you can set the [`htmx.config.noSwap`](https://four.htmx.org/reference/config/htmx-config-noSwap) setting to `[204, 304, '4xx', '5xx']`
- In htmx 2.0, history used a local cache snapshot for history navigation, while in 4.0 it issues a request to the
  ...
  - htmx includes an [`hx-history-cache`](https://four.htmx.org/extensions/hx-history-cache) extension if you wish to have a local cache. This extension integrates
...
For more details see [What's New in 4.0](https://four.htmx.org/docs/whats-new-in-htmx-4)

Extension authors who need to port their extensions to htmx 4 can refer to [Migrating Extensions to 4.0](https://four.htmx.org/docs/extension-htmx-4-migration-guide)
...
- [`hx-get`](https://four.htmx.org/reference/attributes/hx-get) - issues an HTTP `GET`
- [`hx-post`](https://four.htmx.org/reference/attributes/hx-post) - issues an HTTP `POST`
- [`hx-put`](https://four.htmx.org/reference/attributes/hx-put) - issues an HTTP `PUT`
- [`hx-patch`](https://four.htmx.org/reference/attributes/hx-patch) - issues an HTTP `PATCH`
- [`hx-delete`](https://four.htmx.org/reference/attributes/hx-delete) - issues an HTTP `DELETE`
- [`hx-query`](https://four.htmx.org/reference/attributes/hx-query) - issues an HTTP `QUERY`
...
- [`hx-action`](https://four.htmx.org/reference/attributes/hx-action) - The URL to issue a request to
- [`hx-method`](https://four.htmx.org/reference/attributes/hx-method) - The HTTP Action to use
...
You can use triggers to implement many common UX patterns, such as [Active Search](https://four.htmx.org/patterns/active-search):
...
You can also use custom events to trigger requests. Dispatch them with [`htmx.trigger()`](https://four.htmx.org/reference/methods/htmx-trigger)
or from the server with the [`HX-Trigger`](https://four.htmx.org/reference/headers/HX-Trigger) response header.
...
when you are showing the user a [progress bar](https://four.htmx.org/patterns/progress-bar).
...
The two attributes that control this are [`hx-target`](https://four.htmx.org/reference/attributes/hx-target) and [`hx-swap`](https://four.htmx.org/reference/attributes/hx-swap).

#### Targeting Elements

By default, responses target the element that made the request. You can change this by using the
[`hx-target`](https://four.htmx.org/reference/attributes/hx-target) attribute, which takes a [CSS selector](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_selectors) that specifies
...
The [`hx-swap`](https://four.htmx.org/reference/attributes/hx-swap) attribute controls how the content is swapped into (or in place of)
the target element.
...
[`hx-preserve`](https://four.htmx.org/reference/attributes/hx-preserve) to keep an element as it is when an ancestor is replaced:
...
See the [Morphing Guide](https://four.htmx.org/docs/morphing-swaps-guide) for more information on using this technique.

#### Swap Options

The [`hx-swap`](https://four.htmx.org/reference/attributes/hx-swap) attribute also supports options for tuning the swapping behavior of htmx.
...
The [`hx-select`](https://four.htmx.org/reference/attributes/hx-select) attribute takes a plain CSS selector and extracts only that content for
swapping:
...
[`hx-select-oob`](https://four.htmx.org/reference/attributes/hx-select-oob) attribute to do so:
...
If you want to include the values of other elements in a request, you can use the [`hx-include`](https://four.htmx.org/reference/attributes/hx-include) attribute.
...
To send computed values (rather than values held in inputs) use the [`hx-vals`](https://four.htmx.org/reference/attributes/hx-vals) attribute.
...
See [file upload pattern](https://four.htmx.org/patterns/file-upload) for a complete example.
...
the [`hx-validate`](https://four.htmx.org/reference/attributes/hx-validate) attribute on them to "true".
...
To do this, you can add [`hx-swap-oob`](https://four.htmx.org/reference/attributes/hx-swap-oob)`="true"` to an element in your response.
...
[`htmx.config.allowEmptySwapAfterOOB`](https://four.htmx.org/reference/config/htmx-config-allowEmptySwapAfterOOB) to `true` to run the
...
[`hx-indicator`](https://four.htmx.org/reference/attributes/hx-indicator) attribute with an extended CSS selector to do so:
...
In htmx you can "boos" regular HTML anchors and forms using the [`hx-boost`](https://four.htmx.org/reference/attributes/hx-boost) attribute.
...
Consider the [active search](https://four.htmx.org/patterns/active-search) example. As it is written, it will not degrade gracefully:
...
[`hx-post`](https://four.htmx.org/reference/attributes/hx-post) that mirrored the `action` attribute, or perhaps use `hx-boost`
on it.
...
If you want to chante the URL without updating history use the [`hx-replace-url`](https://four.htmx.org/reference/attributes/hx-replace-url)
attribute instead:
...
[`HX-Push-Url`](https://four.htmx.org/reference/headers/HX-Push-Url) and [`HX-Replace-Url`](https://four.htmx.org/reference/headers/HX-Replace-Url) response
headers.
...
[`hx-history-elt`](https://four.htmx.org/reference/attributes/hx-history-elt) attribute:
...
The [`htmx.config.history`](https://four.htmx.org/reference/config/htmx-config-history) setting allow you to specify how history works:
...
[`hx-history-cache`](https://four.htmx.org/extensions/hx-history-cache) extension.
...
If the response code is `400` or `500`, htmx will trigger an [`htmx:response:error`](https://four.htmx.org/reference/events/htmx-response-error) event.

#### Configuring Response Code Handling

You can customize this behavior using the [`hx-status`](https://four.htmx.org/reference/attributes/hx-status) attribute, which takes
...
You can also use the [`htmx.config.noSwap`](https://four.htmx.org/reference/config/htmx-config-noSwap) configuration for global configuration
...
htmx also sends [`HX-History-Restore-Request`](https://four.htmx.org/reference/headers/HX-History-Restore-Request) when it refetches a page
...
To add headers to a request, use the [`hx-headers`](https://four.htmx.org/reference/attributes/hx-headers) attribute.
...
If you want to set headers programmatically, use the [`htmx:config:request`](https://four.htmx.org/reference/events/htmx-config-request) event.
...
The [`HX-Trigger` Response Headers](https://four.htmx.org/reference/headers/HX-Trigger) can be particularly useful, allowing you to trigger
client-side JavaScript code from the server.

### Per-Request Configuration With `hx-config`

The [`hx-config`](https://four.htmx.org/reference/attributes/hx-config) attribute allows you to control fine-grained details of the
...
| Option | Description |
|---------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `timeout` | aborts the request after this time. Accepts `500ms`, `5s`, `2m`, or a number of milliseconds. Defaults to [`htmx.config.defaultTimeout`](https://four.htmx.org/reference/config/htmx-config-defaultTimeout) |
...
The [`hx-sse`](https://four.htmx.org/extensions/hx-sse) extension supports swapping content via these events.
...
For more details, see the [`hx-sse`](https://four.htmx.org/extensions/hx-sse) extension documentation.
...
The [`hx-ws`](https://four.htmx.org/extensions/hx-ws) extension supports swapping content from these messages as well as sending messages
...
For more details, see the [`hx-ws`](https://four.htmx.org/extensions/hx-ws) extension documentation.
...
The [`hx-multipart`](https://four.htmx.org/extensions/hx-multipart) extension supports swapping the content of these parts into the DOM
as they arrive.
...
For more details, see the [`hx-multipart`](https://four.htmx.org/extensions/hx-multipart) extension documentation.
...
Htmx has an [extensive set of events](https://four.htmx.org/reference/events) that you can listen for to log or modify behaviors with:
...
See [htmx Events Guide](https://four.htmx.org/docs/htmx-events-guide) for more details on using htmx events effectively.
...
| Method | Description |
|---------------------------------------------------------------------------------|--------------------------------------------------------------------------|
| [`htmx.ajax()`](https://four.htmx.org/reference/methods/htmx-ajax) | issues an htmx request from JavaScript |
| [`htmx.find()`](https://four.htmx.org/reference/methods/htmx-find) | finds the first element that matches a CSS selector |
| [`htmx.findAll()`](https://four.htmx.org/reference/methods/htmx-findAll) | finds all elements that match a CSS selector |
| [`htmx.process()`](https://four.htmx.org/reference/methods/htmx-process) | initializes htmx attributes on an element and its descendants |
| [`htmx.swap()`](https://four.htmx.org/reference/methods/htmx-swap) | runs the swap lifecycle without a request |
| [`htmx.initialize()`](https://four.htmx.org/reference/methods/htmx-initialize) | initializes htmx manually, if the automatic startup is too early |
| [`htmx.on()`](https://four.htmx.org/reference/methods/htmx-on) | adds an event listener |
| [`htmx.onLoad()`](https://four.htmx.org/reference/methods/htmx-onLoad) | runs a callback when htmx processes new content |
| [`htmx.trigger()`](https://four.htmx.org/reference/methods/htmx-trigger) | dispatches a custom event on an element |
| [`htmx.registerExtension()`](https://four.htmx.org/reference/methods/htmx-registerExtension) | registers an htmx extension |
| [`htmx.parseInterval()`](https://four.htmx.org/reference/methods/htmx-parseInterval) | converts a time string such as `5s` to milliseconds |
| [`htmx.timeout()`](https://four.htmx.org/reference/methods/htmx-timeout) | creates a promise that resolves after a time interval |

The `htmx` object also holds [`htmx.config`](https://four.htmx.org/reference/config/htmx-config), which sets the global configuration.

See the [methods reference](https://four.htmx.org/reference/methods) for the signature and examples of each method.
...
is an [`hx-alpine-compat`](https://four.htmx.org/extensions/hx-alpine-compat) extension that smooths over some corner cases when integrating the two libraries.
...
For more details, see the [`hx-live` Programmers Guide](https://four.htmx.org/docs/hx-live-guide) and the [`hx-live` extension reference](https://four.htmx.org/extensions/hx-live).
...
A good example of this is the [SortableJS demo](https://four.htmx.org/patterns/drag-to-reorder):
...
calling [`htmx.process`](https://four.htmx.org/reference/methods/htmx-process) in the `connectedCallback()` method:
...
If you are using the Shadow DOM in a component, selectors like [`hx-target`](https://four.htmx.org/reference/attributes/hx-target) will
...
|------------------------------------------------------------|----------------|-----------------------------------------------------|
| [`hx-multipart`](https://four.htmx.org/extensions/hx-multipart) | Streaming HTML | Stream HTML with `multipart/mixed` |
| [`hx-sse`](https://four.htmx.org/extensions/hx-sse) | Streaming HTML | Stream HTML with `text/event-stream` (SSE) |
| [`hx-ws`](https://four.htmx.org/extensions/hx-ws) | Streaming HTML | Stream HTML and send data over WebSockets |
| [`hx-browser-indicator`](https://four.htmx.org/extensions/hx-browser-indicator) | UX | Show tab's spinner with `hx-browser-indicator` |
| [`hx-live`](https://four.htmx.org/extensions/hx-live) | UX | Our own DOM-based reactive scripting solution |
| [`hx-pending`](https://four.htmx.org/extensions/hx-pending) | UX | Show custom content during requests |
| [`hx-prompt`](https://four.htmx.org/extensions/hx-prompt) | UX | Prompt before requests with `hx-prompt='Reason?'` |
| [`hx-preload`](https://four.htmx.org/extensions/hx-preload) | Performance | Preload on hover with `hx-preload='mouseover'` |
| [`hx-history-cache`](https://four.htmx.org/extensions/hx-history-cache) | Performance | Restore back/forward pages from `sessionStorage` |
| [`hx-ptag`](https://four.htmx.org/extensions/hx-ptag) | Performance | Skip unchanged polls with `HX-PTag: "v42"` |
| [`hx-download`](https://four.htmx.org/extensions/hx-download) | Swaps | Download files with `hx-swap='download'` |
| [`hx-head`](https://four.htmx.org/extensions/hx-head) | Swaps | Merge `<head>` tags with `hx-head='merge'` |
| [`hx-targets`](https://four.htmx.org/extensions/hx-targets) | Swaps | Target many elements with `hx-targets='.selector'` |
| [`hx-upsert`](https://four.htmx.org/extensions/hx-upsert) | Swaps | Update or insert elements with `hx-swap='upsert'` |
| [`htmx-2-compat`](https://four.htmx.org/extensions/htmx-2-compat) | Compatibility | Restore htmx 2.x defaults and event names on htmx 4 |
| [`hx-alpine-compat`](https://four.htmx.org/extensions/hx-alpine-compat) | Compatibility | Run htmx alongside Alpine.js without conflicts |
| [`hx-csp`](https://four.htmx.org/extensions/hx-csp) | Security | Make htmx work under strict Content Security Policy |

Note that many these extensions are come pre-bundled into [`htmax.js`](https://four.htmx.org/docs/htmax) as a single file.
...
If you wish to build your own htmx extension, see [htmx Extension Authoring Guide](https://four.htmx.org/docs/extension-authoring-guide).
...
the [`ptag` extension](https://four.htmx.org/extensions/hx-ptag).
...
While htmx is simple enough that editor support is not necessary to use it, see [Editor Support](https://four.htmx.org/docs/editor-support)
...
You can set most options directly in JavaScript, or you can use a `meta` tag (accepts [HCON](https://four.htmx.org/docs/hcon-guide) or JSON):
